...
function convertMongoResponse(mongoResponse) {
  return {
    ID: mongoResponse._id,
    first_name: mongoResponse.FirstName,
    last_name: mongoResponse.LastName,
    gpa: mongoResponse.GPA,
    enrolled: mongoResponse.Enrolled
  }
}
...
app.delete('/students/:id', async function (req, res) {
  ...
  res.status(200).send(convertMongoResponse(dataSave));
})
...
app.get('/students', async function (req, res) {
  ...
  // otherwise display "not found" if there are none.
    if (students.length > 0) {
      res.status(200).send(students.map(entry => convertMongoResponse(entry)));
    }
    else {
    ...
    res.status(200).send(dataSave.map(entry => convertMongoResponse(entry)));
  }
})
